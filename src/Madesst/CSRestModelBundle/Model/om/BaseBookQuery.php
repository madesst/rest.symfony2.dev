...
use \PropelPDO;
use Madesst\CsRestModelBundle\Model\Author;
...
abstract class BaseBookQuery extends ModelCriteria
{
    ...
    public function __construct($dbName = null, $modelName = null, $modelAlias = null)
    {
        ...
        if (null === $modelName) {
            $modelName = 'Madesst\\CsRestModelBundle\\Model\\Book';
        }
    ...
    {
        $sql = 'SELECT "id", "title", "author_id" FROM "book" WHERE "id" = :p0';
        try {
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return   \Madesst\CsRestModelBundle\Model\AuthorQuery A secondary query class using the current class as primary query
     */
    public function useAuthorQuery($relationAlias = null, $joinType = Criteria::LEFT_JOIN)
    {
        return $this
            ->joinAuthor($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'Author', '\Madesst\CsRestModelBundle\Model\AuthorQuery');
    }
